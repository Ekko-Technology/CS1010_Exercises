...
do
	...
	then
		make_temp
		timeout ${timelimit[$PROG]} ./"$PROG" < "inputs/$PROG.$i.in" | head -c 50MB > "$out"
		status="${PIPESTATUS[0]}"
		if [ "$status" -ne "0" ]
		then
			if [ "$status" -eq 124 ]; then
				echo "$PROG: took longer than ${timelimit[$PROG]} for test case $i"
			else
			...
			if [ -e "$out" ] 
			then
				if [ "$PROG" == "markup" ] # TODO replace with name of the program as appropriate
				then
...
then
	...
	then
		...
		then
			...
			check_has_used_function "$PROG" isalpha isnum isalnum
	...
	summary
	echo "$PROG: failed $num_failed test case(s):$g_summary"
fi
